...
export const ListTool = Tool.defineEffect(
  ...
  Effect.gen(function* () {
    return {
      ...
      execute: (params: { path?: string; ignore?: string[] }, ctx: Tool.Context<{ count: number; truncated: boolean }>) =>
        Effect.gen(function* () {
          const searchPath = path.resolve(Instance.directory, params.path || ".")
          yield* Effect.promise(() => assertExternalDirectory(ctx, searchPath, { kind: "directory" }))
